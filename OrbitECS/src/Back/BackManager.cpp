...
BackManager::BackManager(HeavyBodies heavy, LightBodies light, BufferExchange& buf,
                          double dt, double simSpeedFactor)
    : heavy_(std::move(heavy)), light_(std::move(light)), buf_(buf),
      dt_(dt), simSpeedFactor_(simSpeedFactor),
      prevHeavyAccel_(heavy_.count_), prevLightAccel_(light_.count_),
      local_{StateSnapshot(heavy_.dynamic_, light_.dynamic_), StateSnapshot(heavy_.dynamic_, light_.dynamic_)} {
    primeAccelerations();
}
...
void BackManager::run() {
    ...
    while (running_) {
        ...
        for (int s = 0; s < stepsPerBatch && running_; ++s) {
            ...
            std::cerr << "ax[Mercury]=" << heavy_.ax[1] << " ay=" << heavy_.ay[1] << "\n"; // debug
    ...
    std::cerr << "[BackManager] Batch de " << stepsPerBatch << " pas calculé en "
              << batchDuration.count() << " ms\n";

        // if (i == 0 || i == 10) {
        //     DebugPrint::exportStepToCsv(heavy_, light_, slot, i, "../../Debug/debug_steps" + std::to_string(i) + ".csv");
        // } else if (i > 10)
        //     exit(0);
        // i++;
